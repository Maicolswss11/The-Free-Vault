-- Ludograph v5.6.3 — fast canonical search results
--
-- v5.6.0 canonicalized the full search result set and timed out on large
-- catalog queries. v5.6.2 restored the fast path but exposed visible store /
-- encyclopedia duplicates again. This migration keeps the candidate-first
-- search plan and canonicalizes only the bounded candidate set before paging.
  ...
  if v_q <> '' then
    ...
    with candidate_keys as materialized (
      ...
      select
        ...
        (
          lower(coalesce(cg.description, '') || ' ' || coalesce(cg.title, '')) ~
          '(port|ports|ported|version|versions|bundle containing|collection of ports|includes? .*(expansion|dlc|undead nightmare)|riedizion|conversion|release for)'
      ...
      select
        f.*,
        case
          when f.is_separate or f.title_key = '' then 'separate:' || f.match_key
          when f.is_variant_candidate then 'title:' || f.title_key || ':' || coalesce(
            ...
            case when f.has_porting_signal then (
              select min(peer.year_value)::text
              from featured peer
              where peer.title_key = f.title_key
                and not peer.is_separate
