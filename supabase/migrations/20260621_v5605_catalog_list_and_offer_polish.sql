-- Ludograph v5.6.5 — catalog list view and offer grouping polish
--
-- v5.6.0 canonicalized the full search result set and timed out on large
-- catalog queries. v5.6.4 restored compact catalog cards but still allowed some bundle/listing
-- records to leak as separate public cards. This migration keeps the
-- candidate-first search plan and folds obvious store offers under visible
-- base works inside the bounded candidate set.
...
begin
  ...
  if v_q <> '' then
    ...
    with candidate_keys as materialized (
      ...
      from eligible e
    ), offer_linked as materialized (
      select
        f.*,
        (
          select peer.title_key
          from featured peer
          where f.is_store_offer
            and peer.title_key <> ''
            and peer.title_key <> f.title_key
            and length(peer.title_key) >= 8
            and not peer.is_store_offer
            and not peer.is_separate
            and f.title_key like '%' || peer.title_key || '%'
          order by length(peer.title_key) desc,
            peer.is_master desc,
            peer.year_value asc nulls last,
            peer.relevance_rank asc
          limit 1
        ) as offer_parent_title_key
      from featured f
      ...
      select
        ...
        case
          ...
          when f.is_store_offer and f.offer_parent_title_key is not null then 'title:' || f.offer_parent_title_key || ':' || coalesce(
            (
              select min(peer.year_value)::text
              from featured peer
              where peer.title_key = f.offer_parent_title_key
                and not peer.is_separate
                and not peer.is_store_offer
    ...
    select
      ...
      jsonb_build_object(
        ...
        'canonicalized', true,
        'canonical_strategy', 'candidate_first_offer_grouping'
      )
